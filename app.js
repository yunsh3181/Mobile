...
function selectionIndicator(){
 ...
 add('피자구성',state.pizzaMode==='half'?'하프앤하프':state.pizzaMode==='single'&&state.step!=='promo'&&state.step!=='setChoice'?'한 판':'',state.pizzaMode==='half'?'◐':'●');
...
function itemCard(x,key,included=false){const q=state[key][x.id]||0;return `<div class="card ${included?'included-card':''} clickable-card" onclick="qty('${key}','${x.id}',1)"><img src="${x.img}" alt="${x.name}"><h3>${x.name}</h3><p class="price">${included?'세트 포함':money(x.price)}</p><div class="qty"><button onclick="event.stopPropagation();qty('${key}','${x.id}',-1)">−</button><strong>${q}</strong><button onclick="event.stopPropagation();qty('${key}','${x.id}',1)">＋</button></div><small class="tap-hint">카드를 터치해 수량 추가</small></div>`}
...
function selectPizzaMode(mode){
 ...
 if(mode==='half'){
  if(state.set===2)return alert('2인 세트는 R 사이즈 전용으로 하프앤하프를 선택할 수 없습니다.');
