...
function itemHTML(item){
 ...
 return `<div class="order-item"><strong>${Number(item.qty)||1}× ${esc(adminPizzaLine(item))}</strong><span>${esc(benefit)}${(item.pizzaMode||item.mode)==='half'?' · 하프앤하프':''}</span>${top?`<small>토핑: ${esc(top)}</small>`:''}${includedSides?`<small>포함 사이드: ${esc(includedSides)}</small>`:''}${includedDrinks?`<small>포함 음료: ${esc(includedDrinks)}</small>`:''}${extraSides?`<small>추가 사이드: ${esc(extraSides)}</small>`:''}${extraDrinks?`<small>추가 음료: ${esc(extraDrinks)}</small>`:''}</div>`;
}
