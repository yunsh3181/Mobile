...
const statusNames={new:'신규',accepted:'접수',cooking:'조리중',ready:'완료',cancelled:'취소'};
...
function filterOrders(order){if(activeFilter==='all')return true;if(activeFilter==='active')return ['new','accepted','cooking'].includes(order.status);return order.status===activeFilter}
function render(){
 const filtered=orders.filter(filterOrders);
 orderList.innerHTML=filtered.length?filtered.map(order=>`<article class="order-card ${order.status}"><div class="order-head"><div><div class="order-no">#${order.orderNo||'-'}</div><small>${formatTime(order.createdAt||order.createdAtClient)}</small></div><span class="status-badge ${order.status}">${statusNames[order.status]||order.status}</span></div><div class="order-meta"><span>${order.orderType==='takeout'?'🥡 포장':'🍽️ 먹고가기'}</span>${order.partySize?`<span>👥 ${order.partySize}명</span>`:''}${order.seat?.name?`<span>🪑 ${order.seat.name}</span>`:''}${order.pickup?.time?`<span>🕒 오늘 ${order.pickup.time}${order.pickup.isHappyHour?' · 해피아워':''}</span>`:''}${order.phoneMasked?`<span>☎️ ${order.phoneMasked}</span>`:''}<span>상품 ${order.itemCount||0}개</span></div><div class="order-items">${(order.items||[]).map(itemHTML).join('')}</div><div class="order-foot"><div class="order-total"><span>결제금액</span><strong>${money(order.total)}</strong></div><div class="actions">${order.status==='new'?`<button class="accept" onclick="setStatus('${order.id}','accepted')">접수</button>`:''}${['new','accepted'].includes(order.status)?`<button class="cook" onclick="setStatus('${order.id}','cooking')">조리 시작</button>`:''}${order.status==='cooking'?`<button class="ready" onclick="setStatus('${order.id}','ready')">조리 완료</button>`:''}${order.status==='ready'?`<button class="call" onclick="callCustomer('${order.orderNo||''}')">📢 고객 호출</button>`:''}${order.status!=='cancelled'?`<button class="cancel" onclick="setStatus('${order.id}','cancelled')">취소</button>`:''}</div></div></article>`).join(''):'<div class="empty">해당 상태의 주문이 없습니다.</div>';
 const count=s=>orders.filter(o=>s.includes(o.status)).length;
 document.getElementById('newCount').textContent=count(['new']);document.getElementById('cookingCount').textContent=count(['accepted','cooking']);document.getElementById('doneCount').textContent=count(['ready']);
...
async function setStatus(id,status){
 try{const order=orders.find(o=>o.id===id);await db.collection('orders').doc(id).update({status,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});if(status!=='new')setTimeout(()=>{if(hasUnacceptedOrders())startNewOrderRepeat();else stopNewOrderRepeat()},300);if(status==='ready'&&order?.seat?.id){await db.collection('seats').doc(order.seat.id).set({status:'cleaning',updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});}if(status==='ready'&&order){playPreset('cafe');speak(`${order.orderNo}번 주문 조리가 완료되었습니다.`)}}catch(error){alert('상태 변경 실패: '+error.message)}
}
...
function hasUnacceptedOrders(){return orders.some(o=>o.status==='new')}
...
function startNewOrderRepeat(){
 ...
 newOrderRepeatTimer=setInterval(async()=>{
   ...
   await playPreset();
   setTimeout(()=>speak('접수되지 않은 신규 주문이 있습니다.'),600);
 },5000);
}

async function notifyNewOrders(added){if(!added.length)return;await playPreset();setTimeout(()=>speak(added.length===1?'새로운 주문이 접수되었습니다.':`신규 주문이 ${added.length}건 있습니다.`),settings.preset==='voice'?0:550);added.forEach(showToast);startNewOrderRepeat()}
...
document.getElementById('previewSound').addEventListener('click',async()=>{settings={preset:soundPreset.value,volume:Number(soundVolume.value)/100,voice:voiceEnabled.checked};if(!soundEnabled){soundEnabled=true;soundButton.textContent='🔔 알림음 켜짐'}await playPreset();setTimeout(()=>speak('새로운 주문이 접수되었습니다.'),settings.preset==='voice'?0:550)});
...
db.collection('orders').orderBy('createdAt','desc').limit(100).onSnapshot(snapshot=>{connectionBadge.textContent='실시간 연결';connectionBadge.className='connection live';const added=[];snapshot.docChanges().forEach(change=>{if(change.type==='added')added.push({id:change.doc.id,...change.doc.data()})});orders=snapshot.docs.map(doc=>({id:doc.id,...doc.data()}));render();if(!initialLoad)notifyNewOrders(added.filter(o=>o.status==='new'));if(soundEnabled&&hasUnacceptedOrders())startNewOrderRepeat();else if(!hasUnacceptedOrders())stopNewOrderRepeat();initialLoad=false},error=>{console.error(error);connectionBadge.textContent='연결 오류';connectionBadge.className='connection error';orderList.innerHTML=`<div class="empty">Firestore 연결 오류: ${error.message}</div>`});
